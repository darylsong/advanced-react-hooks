...
We can use the `useMemo` and `useCallback` hooks to memoize values and
callback functions so as to improve performance. Generally, there are two
reasons why we use these hooks (note that the second reason only applies to
the `useMemo` hook):

1.  Minimising re-renders of components which are memoized using `React.memo`
and which accept objects or functions as props. Memoizing these components
alone do not minimise re-renders if the objects or functions are re-created
by the parent component passing these props. To avoid unnecessary re-renders,
we also have to memoized the props.
2.  To memoize computationally expensive calculations. This does not apply
to the `useCallback` hook because it is generally not expensive to define a
function.
