...
The `useLayoutEffect` hook is similar to the `useEffect` hook, except
that the effects are executed before the mutations to the DOM are
painted to the screen. The general rule for when to use the
`useLayoutEffect` hook is as follows: If you are making observable
changes to the DOM, then it should happen in `useLayoutEffect`,
otherwise `useEffect`.
