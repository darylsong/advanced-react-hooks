...
Apart from the `useState` hook, React also offers the `useReducer` hook for
state management. The general rule for determining when to use `useState` and
when to use `useReducer` are as follows:

1.  When it's just an independent element of state you're managing: `useState`
2.  When one element of your state relies on the value of another element of
